...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 🚨 CRITICAL FIX: Fetch the PREVIOUS avatar BEFORE moderation
    console.log('📥 Fetching previous avatar URL...');
    ...
    if (isRejected) {
      console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      console.log('❌ AVATAR REJECTED - REVERTING TO PREVIOUS AVATAR');
      console.log('Reasons:', rejectionReasons.join(', '));
      console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');

      // Step 1: Delete rejected avatar from Bunny.net Storage
      console.log('🗑️ Step 1: Deleting rejected avatar from Bunny.net Storage...');
      const bunnyDeletionSuccess = await deleteFromBunnyStorage(avatarUrl);
      
      if (bunnyDeletionSuccess) {
        console.log('✅ Rejected avatar deleted from Bunny.net Storage');
      } else {
        console.error('⚠️ Failed to delete rejected avatar from Bunny.net Storage');
      }

      // Step 2: Revert user's avatar to PREVIOUS avatar in database
      console.log('🔄 Step 2: Reverting user avatar to previous avatar in database...');
      console.log('Reverting to:', previousAvatarUrl || 'null (no previous avatar)');
      ...
      if (updateError) {
        console.error('❌ Failed to revert user avatar:', updateError);
      } else {
        console.log('✅ User avatar reverted to previous avatar');
      }
      ...
      const { error: notificationError } = await supabaseClient
        ...
        .insert({
          user_id: userId,
          actor_id: userId,
          type: 'video_rejected', // Using video_rejected type as it's already in the schema
          message: 'Your profile picture was rejected because it contained inappropriate content.',
          is_read: false,
        });
      ...
      console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      console.log('✅ REJECTED AVATAR HANDLED');
      console.log('   - Bunny.net Storage: Rejected avatar deleted');
      console.log('   - Database: Reverted to previous avatar');
...
export default function ProfileScreen() {
  ...
  const checkForRejectedVideos = async () => {
    try {
      ...
      const { data: rejectedVideos, error } = await supabase
        .from('videos')
        .select('id, caption, moderation_status, video_url')
        .eq('user_id', user.id)
  ...
  const handleCancelPendingUpload = async (uploadId: string) => {
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('🚫 CANCELING PENDING UPLOAD FROM PROFILE');
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    ...
    Alert.alert(
      ...
      [
        { text: 'Continue Upload', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              // 1. Get the pending upload details
              ...
              // 2. Check if video was created in database
              if (upload.status === 'completed' || upload.status === 'processing') {
                console.log('🔍 Checking for video in database...');
                
                // Find the video by matching user_id, caption, and approximate creation time
                const { data: videos, error: videoError } = await supabase
                  .from('videos')
                  .select('id, video_url')
                  .eq('user_id', upload.user_id)
                  .eq('caption', upload.caption)
                  .gte('created_at', new Date(new Date(upload.created_at).getTime() - 60000).toISOString()) // Within 1 minute
                  .lte('created_at', new Date(new Date(upload.created_at).getTime() + 300000).toISOString()) // Within 5 minutes
                  .limit(1);
                ...
                if (!videoError && videos && videos.length > 0) {
                  ...
                  if (video.video_url) {
                    ...
                    if (bunnyVideoId) {
                      try {
                        ...
                        console.error('❌ Error deleting from Bunny.net:', deleteError);
                      }
                    }
                  }
                  ...
                  if (dbDeleteError) {
                    console.error('❌ Error deleting video from database:', dbDeleteError);
                  } else {
              ...
              // 3. Delete pending upload record
              console.log('🗑️ Deleting pending upload record...');
              ...
              Alert.alert('Success', 'Upload cancelled and video deleted');
            } catch (error) {
  ...
  const uploadAvatar = async (uri: string) => {
    try {
      ...
      // 🚨 CRITICAL: Trigger AWS Rekognition moderation BEFORE updating database
      ...
      if (moderationError) {
        ...
        // Delete the uploaded avatar from storage
      ...
      if (!moderationData.approved) {
        ...
        // Avatar already deleted by Edge Function, and database already reverted to previous avatar
        // Reload profile to show the reverted avatar
        await loadProfile();
      ...
      // Update database only if approved
  ...
  const handleSaveVideo = async (videoUrl: string, videoId: string) => {
    try {
      ...
      const downloadUrl = await getVideoDownloadUrl(videoUrl, video.library_id);
